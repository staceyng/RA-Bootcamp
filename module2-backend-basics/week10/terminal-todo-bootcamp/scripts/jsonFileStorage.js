 ...
 * @returns undefined
 */
export const add = (filename, key, input, callback) => {
  const readCallback = (writeErr, jsonContentObj) => {
    if (writeErr) {
      console.error("Edit error", err);
      callback(err); //callback function to alter json
      return;
    }

    if (!(key in jsonContentObj)) {
      errMsg = `key -${key} does not exist`;
    ...
    if (typeof input == "number") {
      // complete todo
      const completed_item = jsonContentObj.items[input];
      jsonContentObj.items.splice(input, 1);
      jsonContentObj[key].push(completed_item);
    } else {
      // add todo
  ...
  // edit function
  edit(filename, readCallback, callback);
};
 ...
 * @returns undefined
 */
export const remove = (filename, key, input, callback) => {
  const readCallback = (writeErr, jsonContentObj) => {
    if (writeErr) {
      console.error("Edit error", err);
      callback(err); //callback function to alter json
    ...
    if (typeof input == "number") {
      // remove todo
      const removeItem = jsonContentObj.items[input];
      console.log(`I have removed Item ${input + 1}, "${removeItem}"`);
      jsonContentObj.items.splice(input, 1);
    }
  };
 ...
 * @returns undefined
 */
export const editOneElement = (filename, key, input, payload, callback) => {
  const readCallback = (writeErr, jsonContentObj) => {
    if (writeErr) {
      console.error("Edit error", err);
      callback(err); //callback function to alter json
    ...
    if (typeof input == "number") {
      // edit todo
      const prevItem = jsonContentObj.items[input];
      jsonContentObj.items[input] = payload;
      console.log(
        `I have edited Item ${input + 1}, "${prevItem}" to be "${payload}"`
      );
    }
  };
  edit(filename, readCallback, callback);
