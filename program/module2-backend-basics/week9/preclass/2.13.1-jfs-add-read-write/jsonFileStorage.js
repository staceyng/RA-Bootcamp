...
export function write(filename, jsonContentObj) {
  ...
  const jsonContentStr = JSON.stringify(jsonContentObj, null, 2); // 2 space formatting or \t
  ...
  console.log("write to json file success");
}
...
export function read(filename) {
  const handleFileRead = (readErr, jsonContentStr) => {
    ...
    Object.keys(jsonContentObj).forEach((key) => {
      console.log(key, jsonContentObj[key]);
    });

    console.log("read json file complete");
  };
